    ...
    actions.querySelector('.docs-toggles').onchange = function(event){
        ...
        if(isCheckbox || isRadio){
            ...
            cropper.setCropBoxData(cropBoxData).setCanvasData(canvasData);
                ...
                options.ready = function(){ console.log('ready'); };
            }
            // Restart
            cropper.destroy();
        ...
        if(data.method){
            if(typeof data.target !== 'undefined'){
                ...
                if(!target.hasAttribute('data-option') && data.target && input){
                    try{ data.option = JSON.parse(input.value); } catch(e){ console.log(e.message); }
                }
            }
            switch(data.method){
                case 'rotate':
                if(cropped && options.viewMode > 0){ cropper.clear(); }
                break;
            ...
            switch(data.method){
                case 'rotate':
                if(cropped && options.viewMode > 0){ cropper.crop(); }
                break;
                case 'scaleX':
                case 'scaleY':
            ...
            if(typeof result === 'object' && result !== cropper && input){
                try{ input.value = JSON.stringify(result); } catch(e){ console.log(e.message); }
            }
        }
    };
    document.body.onkeydown = function(event){
        var e = event || window.event;
        if(e.target !== this || !cropper || this.scrollTop > 300){ return; }
        switch (e.keyCode){
            case 37:
            e.preventDefault();
            cropper.move(-1, 0);
