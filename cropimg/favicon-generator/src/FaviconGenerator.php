...
class FaviconGenerator{
     ...
     * Color Teal.
     */
    const COLOR_TEAL = 'FFFFFF';
    /*
     * Color Dark Blue.
     */
    const COLOR_DARKBLUE = 'FFFFFF';
    /*
     * Color LIght Purple.
     */
    const COLOR_LIGHTPURPLE = 'FFFFFF';
    /*
     * Color Dark Purple.
     */
    const COLOR_DARKPURPLE = 'FFFFFF';
    /*
     * Color Dark Red.
     */
    const COLOR_DARKRED = 'FFFFFF';
    /*
     * Color Dark Orange.
     */
    const COLOR_DARKORANGE = 'FFFFFF';
    /*
     * Color Yellow.
     */
    const COLOR_YELLOW = 'FFFFFF';
    /*
     * Color Green.
     */
    const COLOR_GREEN = 'FFFFFF';
    /*
     * Color Blue.
     */
    const COLOR_BLUE = 'FFFFFF';
    /*
    ...
    */
    ...
    final public function __construct($icon = '', $created = false)
    {
        ...
        if (empty($icon)) {
            $icon = "{$this->root}/img/favicon/.original";
        }
        ...
        if (file_exists("{$this->root}/img/favicon/.settings")) {
            $this->settings = json_decode(file_get_contents("{$this->root}/img/favicon/.settings"), true);
        } else {
            $this->settings = array(
        ...
        if (
            file_exists("{$this->root}/img/favicon/.original") === false ||
            filesize($icon) != filesize("{$this->root}/img/favicon/.original")
        ) {
            @mkdir("{$this->root}/img/favicon", 0755);
            @copy($icon, "{$this->root}/img/favicon/.original");
            $this->created == true;
        }
    }
    /**
    ...
    {
        file_put_contents("{$this->root}/img/favicon/.settings", json_encode($this->settings));
    }
    /**
    ...
    final public function createBasic()
    {
        foreach (array('16x16', '32x32', '96x96') as $size) {
            if ($this->created || file_exists("{$this->root}/img/favicon/favicon-{$size}.png") == false) {
                $image = $this->createImage($size);
                $image->writeimage("{$this->root}/img/favicon/favicon-{$size}.png");
            }
        }
        return true;
    }
    ...
    final public function createApple()
    {
        foreach (
            ...
            if ($this->created || file_exists("{$this->root}/img/favicon/apple-touch-icon-{$size}.png") == false) {
                $image = $this->createImage($size);
                $image = $this->setColorAndMargin($image, 'apple-background', 'apple-margin');
                $image->writeimage("{$this->root}/img/favicon/apple-touch-icon-{$size}.png");
            }
        }
        return true;
    }
    ...
    {
        $replace = false;
        $manifest = file_exists("{$this->root}/img/favicon/manifest.json") ?
                            json_decode(file_get_contents("{$this->root}/img/favicon/manifest.json"), true) :
                            array();
        if (
        ...
        foreach (
            ...
            if ($this->created || file_exists("{$this->root}/img/favicon/android-chrome-{$size}.png") == false) {
                $image = $this->createImage($size);
                $image = $this->setColorAndMargin($image, 'android-background', 'android-margin');
                $image->writeimage("{$this->root}/img/favicon/android-chrome-{$size}.png");
            }
            $manifest['icons'][] = array(
                'src'     => "/img/favicon/android-chrome-{$size}.png",
                'sizes'    => $size,
                'type'    => 'image/png',
        ...
        if ($replace && count($manifest) > 0) {
            file_put_contents("{$this->root}/img/favicon/manifest.json", json_encode($manifest));
        }
        return true;
    }
    /**
    ...
    final public function createMicrosoft()
    {
        foreach (array('70x70', '144x144', '150x150', '310x310', '310x150') as $size) {
            if ($this->created || file_exists("{$this->root}/img/favicon/mstile-{$size}.png") == false) {
                if ($size == '310x150') {
                ...
                $image->writeimage("{$this->root}/img/favicon/mstile-{$size}.png");
            }
        }
        if (file_exists("{$this->root}/img/favicon/browserconfig.xml") === false || $this->created) {
            $browserconfig =
...
<browserconfig>
    <msapplication>
        <tile>
            ...
            file_put_contents("{$this->root}/img/favicon/browserconfig.xml", $browserconfig);
        }
        return true;
    }
    /**
    ...
    final public function getHtml()
    {
        $html = '';
        foreach (array('16x16', '32x32', '96x96') as $size) {
            if (file_exists("{$this->root}/img/favicon/favicon-{$size}.png")) {
                $html .= "<link rel=\"icon\" type=\"image/png\" href=\"/img/favicon/favicon-{$size}.png\" sizes=\"{$size}\">\n";
            }
        }
        foreach (
            array('57x57', '60x60', '72x72', '76x76', '114x114', '120x120', '144x144', '152x152', '180x180')
            ...
            if (file_exists("{$this->root}/img/favicon/apple-touch-icon-{$size}.png")) {
                $html .= "<link rel=\"apple-touch-icon\" sizes=\"{$size}\" href=\"/img/favicon/apple-touch-icon-{$size}.png\">\n";
            }
        }
        if (file_exists("{$this->root}/img/favicon/android-chrome-192x192.png")) {
            $html .= "<link rel=\"icon\" type=\"image/png\" href=\"/img/favicon/android-chrome-192x192.png\" sizes=\"192x192\">\n";
        }
        if (file_exists("{$this->root}/img/favicon/manifest.json")) {
            $html .= "<link rel=\"manifest\" href=\"/img/favicon/manifest.json\">\n";
        }
        if (file_exists("{$this->root}/img/favicon/mstile-144x144.png")) {
            $html .= "<meta name=\"msapplication-TileImage\" content=\"/img/favicon/mstile-144x144.png\">\n";
        }
    ...
    final private function createImage($size)
    {
        ...
        $original = new Imagick("{$this->root}/img/favicon/.original");
