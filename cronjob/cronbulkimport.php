        ...
        for($row =2; $row <= $total_rows; $row++){
            ...
            //Print each cell of the current row
             $ID=ucwords(trim($single_row[0][0]));   //Product/Service
                ...
                $sale_start= trim($single_row[0][17]);
                $sale_end= trim($single_row[0][18]);
                ...
                $cod= trim($single_row[0][25]);
                 $codcharges= trim($single_row[0][26]);
                $seotitle= trim($single_row[0][27]);
                $seodesc= trim($single_row[0][28]);
                $seokeywords= trim($single_row[0][29]);
                $isActive= trim($single_row[0][30]);
                $isDelete= trim($single_row[0][31]);
                $img1= $single_row[0][32];
                $img2= $single_row[0][33];
                $img3= $single_row[0][34];
                $img4= $single_row[0][35];
                $proddesc=trim($single_row[0][36]);
                         ...
                         if($prod->checkprod($productname,$uploader,$ID)){
                            ...
                            $action= ($ID!=0&&$ID!=""?"Update":"Add");
                             $prodbasic['prod_name']=trim($productname);
                              ...
                              $prodbasic['cod_charges']=($cod!=""&&$cod!=0?$codcharges:0);
                              $prodbasic['vendor_reg_price']=$price;
                              $prodbasic['vendor_sale_price']=$sale_price;
                              $prodbasic['vendor_sale_start_date']=($sale_start!=""?date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($sale_start)):"");
                              $prodbasic['vendor_sale_end_date']=($sale_end!=""?date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($sale_end)):"");
                               $prodbasic['seo_title']=($seotitle!=""?$seotitle:trim($productname));
                              $prodbasic['seo_description']=($seodesc!=""?$seodesc:trim($productname));
                              $prodbasic['seo_keywords']=($seokeywords!=""?$seokeywords:trim($productname));
                              ...
                              if($action=="Add"){
                                  ...
                                  $prodbasic['final_price']=(($sale_start!=""&&$sale_end!="")&&(date("Y-m-d H:i:s")>=date("Y-m-d H:i:s",strtotime(date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($sale_start))))&&date("Y-m-d H:i:s")<=date("Y-m-d H:i:s",strtotime(date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($sale_end)))))?$sale_price:$mrp);
                                  }else{
                               ...
                               if($mainprodid){
                                ...
                                for($t=36;$t<count($mainhead);$t++){
                                    $atr=explode("(",$mainhead[$t]);
                                    $fldname=str_replace(" ","",trim($atr[0]));
                                   $where=" field= '".$fldname."'";
            ...
            //Print each cell of the current row
             $ID=ucwords(trim($single_row[0][0]));   //Product/Service
                ...
                $sale_start= trim($single_row[0][19]);
                $sale_end= trim($single_row[0][20]);
                ...
                $codcharges= trim($single_row[0][28]);
                $seotitle= trim($single_row[0][29]);
                $seodesc= trim($single_row[0][30]);
                $seokeywords= trim($single_row[0][31]);
                $isApproved= trim($single_row[0][32]);
                $newarrival= trim($single_row[0][33]);
                $trendong= trim($single_row[0][34]);
                $isactive= trim($single_row[0][35]);
                $isDelete= trim($single_row[0][36]);
               $img1= $single_row[0][37];
               $img2= $single_row[0][38];
               $img3= $single_row[0][39];
               $img4= $single_row[0][40];
              $proddesc=trim($single_row[0][41]);
                $prodbasic=array();   $templatefld=array();
                $variations=$clprod->exvariation2($variationvalues);  $varon= $clprod->exvariation2($variationOn);
                    ...
                    if($prod->checkCat($cat)=="Invalid"){ $flag= 0; }
                            ...
                            $action= ($ID!=0&&$ID!=""?"Update":"Add");
                             ...
                             $prodbasic['variant_value3']=(count($variations)>=3?trim($variations[2]):"");
                              ...
                              $prodbasic['seo_description']=($seodesc!=""?$seodesc:trim($productname));
                              $prodbasic['seo_keywords']=($seokeywords!=""?$seokeywords:trim($productname));
                              $prodbasic['final_price']=($admin_price!=0&&$admin_price!=""?$admin_price:(($sale_start!=""&&$sale_end!="")&&(date("Y-m-d H:i:s")>=date("Y-m-d H:i:s",strtotime(date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($sale_start))))&&date("Y-m-d H:i:s")<=date("Y-m-d H:i:s",strtotime(date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($sale_end)))))?$sale_price:$mrp));
                              $prodbasic['weight']=$weight;  $prodbasic['length']=$length; $prodbasic['height']=$height; $prodbasic['width']= $width;
