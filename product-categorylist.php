...
$prod = new Product(); $categorydetails = $prod->getcategoryDetails($urltitle); $where = 'master_cat='.$categorydetails[0]['id']; $pagetitle = $categorydetails[0]['page_title']; $bread =  $prod-> getBreadcrumb("master_cat",$categorydetails[0]['id']); ?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <meta property="og:title" content="<?=($pagetitle!=""?$pagetitle." : ".SITE_TITLE:$bread['parent']." - ".$bread['master']." : ".SITE_TITLE); ?>">
    ...
    <meta property="twitter:title" content="<?=($pagetitle!=""?$pagetitle." : ".SITE_TITLE:$bread['parent']." - ".$bread['master']." : ".SITE_TITLE); ?>">
...
<div class="main-wrap pb-4">
    ...
    <div class="container">
        <ol class="breadcrumb mb-0 pl-0 pb-0 mb-3"><li class="breadcrumb-item"><a href="<?=SITEURL; ?>" hreflang="en">Home</a></li><li class="breadcrumb-item"><?=$bread['master']; ?></li></ol>
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-12 col-lg-12">
                ...
                <? $parent = selectQuery(PRODCAT,"cat_name,img_name,id,type,url_title","type='Master' AND isActive='1' AND parent_id=".$categorydetails[0]['id']." order by priority");
                if(count($parent)){ ?>
                            ...
                            $parenttype = $parent[$p]['type']; $parentid=$parent[$p]['id'];
                             $checksub = selectQuery(PRODCAT,"count(id) as sub","type='Sub' AND isActive='1' AND parent_id=".$parentid." AND img_name<>''");

                            if($checksub[0]['sub']!=0){ $urlsrc=SITEURL."/s/".$parent[$p]['url_title']; }
                            else{$urlsrc=SITEURL.'/'.getUrl("Master",$parentid);}
                            ?>
                            <div class="col-4 col-sm-4 col-md-4 col-lg-2 mb-3 text-center pr-0"><a href="<?=$urlsrc;?>" hreflang="en"><div class="pro-cat-tumb categ-item position-relative shop-by-cat d-flex flex-wrap align-content-center card rounded-top p-2 p-md-3"><img src="<?=$src; ?>" alt="<?php echo $parent[$p]['cat_name'] ?>" class="mw-100 m-auto rounded"></div>
