<div id="order-item" class="modal fade" role="dialog"><div class="modal-dialog modal-lg delved-order-pop"><div class="modal-content item_detail"></div></div></div>
            ...
            <div class="modal-body p-4">
                ...
                <form>
                    ...
                    <div class="custom-control custom-radio mb-2">
                        <input class="custom-control-input" id="prc-to-high" name="cancel_reason" type="radio" value="Vendor is not available">
                        <label class="custom-control-label" for="prc-to-high">Vendor is not available</label>
                    </div>
                    <div class="custom-control custom-radio mb-2">
...
function get_item(itemid){ 
    $.ajax({
        url : "<?php echo ADMINURL ?>/order/ajaxdata.php",
        type : "post",
        ...
        success: function(response){
            if(response){
                ...
                for(i=0;i<jssonarr.length;i++){
                    str+='<div class="col-sm-12 py-2 border-bottom"><span class="small">'+jssonarr[i]["date"]+'</span><div>'+jssonarr[i]["activity"]+'</div> <div>'+jssonarr[i]["location"]+'</div> </div>'
                } 
                str+='</div>';
                $("#"+divid).html(str)
            }
...
$(".track-ord-status").click(function(){
    $(this).parents(".order-status-col").next().slideToggle();
});
...
function confirm_alertbox(id,txnId,itemId,canc_charge){
 ...
 function confirmdelivered(itemid){
    ...
    del_alertbox(msg, itemid,"deliveredOrder");
 }

 function deliveredOrder(item){
    info = {item:item,action:"deliveredOrder"}
    $.ajax({
        type:"POST",
        url:"<?=SITEURL; ?>/ajax/order_ajax.php",
        data:info,
        ...
        response=response.trim();
            if(response== "1"){
                $('.alert_msgs').fadeIn().removeClass("failactionmsg").addClass("successactionmsg").html("Order marked as delivered").delay(3000).fadeOut();
                $("#del_popup").modal("hide");
                $("#inprocess").load(" #inprocess");
                $("#hyperlocal").load(" #hyperlocal");
...
function cancelOrder(shippingOrderId,txnId,itemId){
    ...
    if(cancel_reason  == "" || cancel_reason == undefined ) {
        ...
        var info ={shippingOrderId:shippingOrderId,txnId:txnId,itemId:itemId,cancel_reason:cancel_reason,action:"cencelOrder",canceled_by:'Admin'};
        $.ajax({
            type: "POST",
            url: "<?=SITEURL; ?>/ajax/order_ajax.php",
            data:info,
