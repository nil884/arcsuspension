...
<div class="page-body-wrapper">
    ...
    <div class="main-panel">
        <div class="dashbody">
            <div class="card">
                <div class="card-header sec-card-head justify-content-between align-items-center"><div><h2 class="card-head-title">FAQ</h2></div></div>
                <form>
                    <div class="card-body">
                        <div class="row">
                            <label class="col-sm-12 control-label">Description</label>
                            <div class="col-md-12"><textarea class="form-control summernote" id="Faq" maxlength="500" placeholder="Enter information(Max 500 Characters)" required><?php echo $getdetails[0]['Faq']; ?></textarea></div>
                        </div>
                    </div>
                    <div class="card-footer text-right py-2"><button type="button" class="btn btn-primary" id="Faq_submit">Submit</button></div>
                </form>
            </div>
        </div>
...
$(document).ready(function(){
    $("#Faq_submit").on("click", function(){
        var Faq = $("#Faq").val();
        var cleandata = $("#Faq").summernote("code").replace(/<\/?[^>]+(>|$)/g, "");
        if(cleandata.trim().length){ var data_count=1 }else{var data_count=0 }
        var info = { Faq: Faq, data_count:data_count, action:'Faq' };
        $.ajax({
            type: "POST",
            url: "ajaxdata.php",
            data: info,
