...
<div class="page-body-wrapper">
    ...
    $stockalert = selectQuery(PRODINFO,"id,prod_name,stock,sold", "stock-sold <= ".STOCK_ALERT." and stock <> 0   order by stock-sold asc"); ?>
            ...
            <div class="card mb-0">
                ...
                <div class="card-body">
                    <table id="recent-orders2" class="table table-bordered ps-container ps-theme-default w-100">
                        <thead><tr><th class="row-index-tr">#</th><th>Products</th><th style="width:40px;" class="text-center">Stock</th></tr></thead>
                        <tbody>
                                    <?php if(count($stockalert)){
                                    for($i=0;$i<count($stockalert);$i++){  $r=$stockalert[$i]; ?>
                                    <tr> <td><?=$i+1; ?></td> <td><a href="<?=ADMINURL; ?>product/view-details.php?prod_id=<?=base64_encode($r['id']); ?>" target="_blank"><?php echo $r['prod_name'] ?></a></td>
