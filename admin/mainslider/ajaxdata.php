...
if($action == "Delete_image"){
    $imgid = $_POST['imgid'];
    $del_img =  deleteQuery(MAIN_SLIDER,'img_id="'.$imgid.'"');
    if($del_img){
        deleteimg('main slider',$_REQUEST['img_name'] );
        echo "1";
    } else{echo "0";}
}
if($action == "priority_image"){
    ...
    if ($str != ""){
        ...
        $data = array('priority' => $i + 1);
            $update = updateQuery(MAIN_SLIDER, $data, "img_id=" .$image_ids[$i]);
...
if($action == "update_data"){
    ...
    $getuser = updateQuery(MAIN_SLIDER,$data,"img_id=".$_REQUEST['img_id']);
