...
<html lang='en'>
<head>
    <title><?php echo SITENAME; ?> : Update Blog Images</title>
...
<div class="page-body-wrapper">
    ...
    $getconfingdetails = json_decode(getimgconfig('blog'));
    $getblogimgs = selectQuery(BLOGIMG,"id,img_name"," blogid = '".base64_decode($_REQUEST['blogid'])."' order by id DESC");
    $img_location = $getconfingdetails[0]->imgs_location;
    $getconfingdetails = getimgconfigpaths('blog'); ?>
        ...
        <div class="dashbody">
            ...
            <div class="card mb-0">
                <div class="card-header sec-card-head justify-content-between align-items-center py-2">
                    <div><h2 class="card-head-title mb-2 mb-lg-0">Images - <?php echo $basicdetail[0]['title'];?></h2></div>
                ...
                <div class="card-body">
                    ...
                    <div id="imgdata">
                        <input type="hidden" id="img_count" value='<?php echo count($getblogimgs); ?>'>
                        <div class="row">
                            <!-- <a href="uploadimg.php?prodid=<?php echo  base64_encode($getinv[$i]['id']) ?>">text</a>-->
                            <?php if(count($getblogimgs)){
                            for($j=0;$j<count($getblogimgs);$j++){  ?>
                            <div class="col-8 col-sm-4 col-md-3 col-lg-3 col-xl-3">
                                <div class="position-relative uped-prod-thumb border p-2 mb-2">
                                    <img src="<? echo SITEURL."/".$img_location."/".$getblogimgs[$j]['img_name']; ?>" alt="" class="img-fluid"/>
                                    <span class="removeopt pro-attr-badge-action del-upload-pic shadow-sm btn btn-danger p-1 rounded-circle" onclick="del_blog_img('<?php echo $getblogimgs[$j]['id'] ?>','<?php echo $getblogimgs[$j]['img_name'] ?>') "><i class="fa fa-trash"></i></span>
                                </div>
                            </div>
...
function del_blog_img(imgid,type){ del_alertbox("Do you really want to delete this Image?", imgid,"del_image_db",type); }
