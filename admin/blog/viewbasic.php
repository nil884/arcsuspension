...
<html lang='en'>
<head>
    <title><?php echo SITENAME; ?> : Update Blog</title>
...
<div class="page-body-wrapper">
    ...
    <div class="main-panel">        
        <div class="dashbody">
            ...
            <div class="card mb-0">
                <div class="card-header sec-card-head justify-content-between align-items-center py-2"><div><h2 class="card-head-title">Update Blog</h2></div><div class="btn-actions-pane-right"><button onclick="goBack()" class="btn btn-secondary btn-sm">Back</button></div></div>
                <div class="card-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9 col-md-9 col-lg-10" id="catbox">
                            <select class="form-control" name="category" id="category" onchange="catchng()">
                                ...
                                <option value="<?php echo $getcat[$i]['cat_id']; ?>" <?php if($getcat[$i]['cat_id'] == $basicdetail[0]['category']) { echo "selected"; } ?>><?php echo $getcat[$i]['category_name']; ?> </option>
                                <? } ?>
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-sm-9 col-md-9 col-lg-10"><input type="text" class="form-control" id="title" name="title" value="<?php echo $basicdetail[0]['title']; ?>" placeholder="Blog Title Max 600 Character Limit"></div>
                        </div>
                    </div>
                    <div class="row">
                            ...
                            <div class="row mr-0">
                                <div class="col-6 pr-0">
                                    <div class="form-group">
                                        ...
                                        <?php $posted_date = explode ("-" , $basicdetail[0]['posted_date']);
                                        $postdate = $posted_date[2]."-".$posted_date[1]."-".$posted_date[0]; ?>
                                        <input type="text" name="pd1" placeholder="Posted date" class="form-control border-right-0" id="datepicker1" value="<?php echo $postdate; ?>">
                                        <div class="input-group-append"><span class="input-group-text bg-white"><span class="fa fa-calendar"></span></span></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-6 pr-0"><div class="form-group"><input type="text" class="form-control" id="posted_by" name="posted_by" placeholder="Posted By" value="<?php echo $basicdetail[0]['posted_by']; ?>"></div></div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <label class="col-sm-3 col-md-3 col-lg-2 col-form-label cc-mandatorystar">Details</label>
                        <div class="col-sm-9 col-md-9 col-lg-10"><textarea class="summernote" id="Summary" name="Summary"><?php echo $basicdetail[0]['summary']; ?></textarea></div>
                    </div>                    
                </div>
                <div class="card-footer py-2 text-right"><button type="button" class="btn btn-primary addit" onclick="updateblock()">Update Blog</button></div>
            </div>
        </div>
    ...
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div><input type="text" name="newcat" id="newcat" placeholder="New Category Max 200 Character Limit" class="form-control text-capitalize" maxlength="50"/></div>                
            </div>
            <div class="modal-footer"><input type="button" id="addnewscat" value="Add New Category" class="btn btn-primary" /></div>
        </div>
    </div>
</div>
...
function updateblock(){
    ...
    if((Summaryhtml == "") || (title == "")  || (posted_date == "")  || (posted_by == "") || (category == "")|| (category == "addnewcat")){
        if((category == "")|| (category == "addnewcat")){
            $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html("Please select category").delay(3000).fadeOut();
        } else if (title == ""){
            $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html("Please enter title").delay(3000).fadeOut();
        } else if (Summaryhtml == ""){
            $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html("Please enter summary").delay(3000).fadeOut();
        } else if(posted_date == ""){
            $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html("Please enter posted date").delay(3000).fadeOut();
        } else if(posted_by == ""){
            $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html("Please enter posted by name").delay(3000).fadeOut();
        }
    } else{
        $(".addit").html("Updating...").attr('disabled',true);
        $.ajax({
            type: "POST",
            url: "ajaxdata.php",
            data: form_data,
            ...
            success: function(response){
                $(".addit").html("Update Blog").attr('disabled',false);
                $(".addit img").remove();
                if (response.trim() == "0"){
                    ...
                    $('.alert_msgs').fadeIn().removeClass("failactionmsg").addClass("successactionmsg").html('Blog updated successfully').delay(3000).fadeOut();
                }
                else if(response.trim() == "2"){
                    $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html('Blog with same title already Exist').delay(3000).fadeOut();
                }
            }
        })
    }
...
$("#addnewscat").click(function(){
    ...
    if(newcat == ""){
        $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html("Details are not available").delay(3000).fadeOut();
    } else {
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                if(response==0){ $('.alert_msgs').fadeIn().removeClass("successactionmsg").addClass("failactionmsg").html("ERROR..").delay(3000).fadeOut();
                } else if(response==1){
