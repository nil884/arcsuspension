...
class ProductsSample extends BaseSample {
 ...
 /*  public function run($prod) {
    ...
    $exampleProduct = $this->createProduct($prod);
  ...
  public function createProduct($prod) {
    $id=$prod['id']; $title=$prod['title']; $description=$prod['description'];  $produrl=$prod['produrl']; $img=$prod['img'];$prodprice=$prod['price']; $availability=$prod['availability']; $sku=$prod['sku'];
    $item_group_id=$prod['item_group_id']; $color=$prod['color']; $size=$prod['size']; $material=$prod['material']; $pattern=$prod['pattern'];
    $product = new Google_Service_ShoppingContent_Product();
  
    $product->setOfferId($id);
    $product->setTitle($title);
    $product->setDescription(($description?$description:$title));
    $product->setLink($produrl);
    $product->setImageLink($img);
    ...
    //$product->setGtin('9780007350896');
    if($item_group_id!=""){$product->setItemGroupId($item_group_id);}
    if($color!=""){$product->setcolor($color);}
    if($size!=""){$product->setSizes($size);}
    if($material!=""){$product->setMaterial($material);}
    if($pattern!=""){$product->setPattern($pattern);}
  ...
  private function createExampleProduct($offerId) {
    ...
    $product->setTitle('A Tale of Two Cities pune');
